...
C-----------------------------------------------
     ...
     3                      tprec2d = 12
      ...
      REAL(rprec1) :: tvacon, tvacoff, tfunon, tfunoff,
     ...
     5               treson, tresoff, tprec2don, tprec2doff
      REAL(rprec1), DIMENSION(0:12) :: timer
      ...
      INTEGER             :: i, nform
      CHARACTER*(*), DIMENSION(0:12), PARAMETER :: form =
     ...
     Z     /)
      ...
      timer(tfun) = timer(tfun) - timer(tvac) - timer(trecon)
     ...
     2            - timer(tbcov) - timer(tres)
      ...
      DO i = 1,2
         ...
         IF (lfreeb .and. lrecon) THEN
            WRITE (nform, 20)
     1            form(tsum) ,timer(tsum) , form(tread) ,timer(tread), 
     ...
     5            form(tres), timer(tres) 
         ...
         WRITE (nform, 20) form(tfun), timer(tfun)
      END DO
