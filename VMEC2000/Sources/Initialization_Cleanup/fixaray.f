...
!
      mnyq = 2*MAX(0, ntheta1/2, mpol1)           !!2 X Nyquist (max) frequency: needed in jxbforce aliasing
      nnyq = 2*MAX(0, nzeta/2, ntor)
     ...
     2         mscale(0:mnyq), nscale(0:nnyq), stat=istat2)
      ...
      mscale(0) = 1           !osqrt2
      nscale(0) = 1           !osqrt2
      mscale(1:mnyq) = SQRT(two)*mscale(0)
      nscale(1:nnyq) = SQRT(two)*nscale(0)
