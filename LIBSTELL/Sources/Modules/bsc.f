...
! SECTION VIII. VECTOR POTENTIAL SUBROUTINES
! SECTION IX. MAGNETIC FIELD SUBROUTINES
! SECTION X.  MAGNETIC FLUX INTEGRAL SUBROUTINES
! SECTION XI. AUXILIARY SUBROUTINES
! SECTION XII.  DEBUGGING SUBROUTINES
! SECTION XIII.  SPECIAL FUNCTIONS
! SECTION XIV.  DUPLICATE CODING FOR TESTING
!*******************************************************************************
      MODULE bsc
...
!  Logic to determine nm1
!  For fil_loops, assume that xnod argument is not closed.
!     If n_xnod_2 .eq. 2, then only two nodes, i.e. a single straight line
!     filament. Don't close in this case.
!     Close for all other fil_loops.
!  For 'fil_rog'owskis, don't close
!  Use n_xnod_2 to refer to SIZE of xnod argument
!  Use n and nm1 to refer to SIZE of this component
         IF ((c_type .eq.'fil_loop') .or. (c_type .eq.'floop')) THEN
            IF (n_xnod_2 .eq. 2) THEN
               WRITE (6,*) ' WARNING: bsc_construct: one straight ',           &
     &                  ' filament for coil: ', TRIM(s_name)
               WRITE (6,*) ' (this is a straight coil filament)'
!  Do NOT close filament (not a true loop)
               nm1 = n_xnod_2 - 1
            ELSE
               nm1 = n_xnod_2
            ENDIF
         ELSE IF (c_type .eq.'fil_rogo') THEN
            nm1 = n_xnod_2 - 1
         ENDIF
...
!  Close up the coil (if n > n_xnod_2). Otherwise read in end point
         IF (n .gt. n_xnod_2) THEN                      
            this % xnod(1:3,n) = xnod(1:3,1)               
         ELSE
            this % xnod(1:3,n) = xnod(1:3,n)
         END IF
...
!-------------------------------------------------------------------------------
      SUBROUTINE bsc_fluxba_coil(coil_a,coil_b,flux,bsc_k2)
      IMPLICIT NONE
...
!  avecs.
      CALL bsc_flux_pos(coil_b,positions,tangents,avecs,npoints)
...
!-------------------------------------------------------------------------------
      SUBROUTINE bsc_fluxba_coil_a(coil_a,coil_b,flux,bsc_k2)
      IMPLICIT NONE
...
!-------------------------------------------------------------------------------
      SUBROUTINE bsc_fluxba_coilcoll(coil_a,coil_b,flux,bsc_k2)
      IMPLICIT NONE
...
!-------------------------------------------------------------------------------
      SUBROUTINE bsc_flux_pos(coil_b,positions,tangents,avecs,npoints)
      IMPLICIT NONE
...
!  For fil_loops, one point, in the middle of each filamentary segment
      SELECT CASE (coil_b % c_type)
      CASE ('fil_loop','floop','fil_rogo')
         DO i = 1,npoints
            positions(1:3,i) = coil_b % xnod(1:3,i) + 0.5 *                    &
     &         coil_b % dxnod(1:3,i)
            tangents(1:3,i) = coil_b % dxnod(1:3,i)
         END DO
      CASE ('fil_circ','fcirc')
...
!*******************************************************************************
! SECTION XIII.  SPECIAL FUNCTION(S)
!*******************************************************************************
...
!  File 6.4 - eliminated.
!*******************************************************************************
! SECTION XV.  COMMENTS FOR DIFFERENT REVISIONS
!*******************************************************************************
!  
!  JDH 10.03.02
