...
!  TYPE prfun is 247+8*size(a) bytes (+ size of kp_store, JDH)
      TYPE prfun
         ...
         CHARACTER (len=63) :: name_diagnostic_dot
...
!-----------------------------------------------
      ...
      CHARACTER*1, OPTIONAL, INTENT(in) :: mmode
...
!-----------------------------------------------
     ...
     &   stat = istat)
      IF (istat .ne. 0) THEN
          WRITE(*,*) 'In cdf_crfun_read, istat = ', istat
          RETURN
       END IF
...
! cdf_prfun_read reads NETCDF file of vector potential for sinle sensor
