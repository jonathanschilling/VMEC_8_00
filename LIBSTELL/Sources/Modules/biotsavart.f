...
C-----------------------------------------------
      ...
      TYPE (bsc_coilcoll), DIMENSION(:), ALLOCATABLE :: coil_group
...
!
      ...
      SUBROUTINE initialize_biotsavart (extcur_in, extension, xpt)
      IMPLICIT NONE
C-----------------------------------------------
...
C-----------------------------------------------
      ...
      INTEGER            :: istat
      INTEGER(iprec)     :: nextcur, nfp, ig, nc
...
!  Parse coils.extension file and initialize bsc routines
         ...
         CALL parse_coils_file (TRIM(coil_file), nfp)
...
!  Create the coil (DO NOT CLOSE IT!)
     ...
     &         extcur_in(1),xpt(1:3,1:nc))
      ...
      SUBROUTINE parse_coils_file (coil_file, nfp, lgrps)
      USE safe_open_mod
      IMPLICIT NONE
!-----------------------------------------------
...
C   L o c a l   P a r a m e t e r s
C-----------------------------------------------
      INTEGER(iprec), PARAMETER :: icoil0=22
      INTEGER(iprec), PARAMETER :: maxgroups = 1000
C-----------------------------------------------
C   L o c a l   V a r i a b l e s
C-----------------------------------------------
      INTEGER            :: istat
      INTEGER(iprec), DIMENSION(:) ::  ngroup(maxgroups)
      INTEGER(iprec)     :: nmaxnodes, icoil, nextcur
      CHARACTER(LEN=200) :: line, group
      LOGICAL            :: local_lgrps
C-----------------------------------------------
!
!     OPEN COILS FILE AND READ IN NUMBER OF FIELD PERIODS
!
      icoil = icoil0
...
!     and the maximum number of nodes in ANY one coil.
      CALL read_coils_pass1(icoil, nextcur, nmaxnodes, ngroup, 
     1                      local_lgrps)
...
!     them in the appropriate groups.
      CALL read_coils_pass2(icoil, nmaxnodes, coil_group, ngroup,              &
     &                      local_lgrps)
...
!-----------------------------------------------
      ...
      INTEGER(iprec), INTENT(out) :: n_coilgroups, nmaxnodes
      INTEGER(iprec), DIMENSION(:), INTENT(out) :: ngroup
...
!           MIGHT occur in file and should be accounted for in this logic.
         ...
         READ(line,*,IOSTAT = istat) xw, yw, zw, currin,                       &                       
     &     igroup, group_id
         IF (istat .eq. 0) THEN 
...
100   WRITE(6,*) 'Problems in read_coils_pass1'
...
!-----------------------------------------------
      INTEGER, INTENT(in) :: iou
      INTEGER(iprec), INTENT(in) :: nmaxnodes
      INTEGER(iprec), DIMENSION(:), INTENT(in) :: ngroup
...
!-----------------------------------------------
      ...
      INTEGER            :: istat
      INTEGER(iprec)     :: n_coilgroups, id_group
      INTEGER(iprec)     :: i, igroup, inodes, nnod, icoil
      TYPE(bsc_coil)     :: coil_temp
...
!     Reread the line
         READ(line,*) xnod_in(1:3,inodes), currin
...
!     EXCEPTION: two-point filament (infinite straight coil)
            nnod = inodes - 1
            IF (inodes .eq. 2) nnod = inodes
...
100   WRITE(6,*) 'Problems in read_coils_pass2'
      WRITE(6,*) 'eof before END'
