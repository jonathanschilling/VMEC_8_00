...
! Variable names (vn_...) : put eventually into library, used by read_wout too...
     ...
     6  vn_vol = 'volume_p', vn_mse = 'imse', vn_thom = 'itse',
...
!-----------------------------------------------
     ...
     3   specw, jdotb, bdotgradv, fsqt, wdot,
     ...
     6   qmeas, datastark, rthom, datathom, dsiobt
      ...
      CHARACTER :: mgrid_file*100, input_extension*100
...
C-----------------------------------------------
      ...
      CHARACTER*(*), INTENT(in) :: file_or_extension
C-----------------------------------------------
...
!
     ...
     1        nobd, nextcur
      ...
      IF (ierr_vmec.ne.0 .and. ierr_vmec.ne.4) GOTO 1000
     ...
     1  lmns(mnmax,ns), bmnc(mnmax,ns), gmnc(mnmax,ns), 
     2  bsubumnc(mnmax,ns),
     2  bsubvmnc(mnmax,ns), bsubsmns(mnmax,ns), bsupumnc(mnmax,ns),
     3  bsupvmnc(mnmax,ns), currvmnc(mnmax,ns), 
     4  iotas(ns), mass(ns),
     4  pres(ns), beta_vol(ns), phip(ns), buco(ns), bvco(ns), phi(ns),
     5  iotaf(ns), presf(ns), phipf(ns),
     ...
     2             bmns(mnmax,ns), gmns(mnmax,ns), bsubumns(mnmax,ns), 
     3             bsubvmns(mnmax,ns), bsubsmnc(mnmax,ns), 
     4             bsupumns(mnmax,ns), bsupvmns(mnmax,ns), 
     5             stat=istat(6))
...
! Read Arrays
      ...
      raxis(:,1) = raxis_cc;   zaxis(:,1) = zaxis_cs
      raxis(:,2) = 0;          zaxis(:,2) = 0
...
!-----------------------------------------------
     ...
     3  pres, beta_vol, phip, buco, bvco, phi, vp, jcuru,
